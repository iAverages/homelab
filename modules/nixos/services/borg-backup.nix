...
in {
  ...
  config.services.borgbackup.jobs.daily = lib.mkIf config.system.borgbackup.daily.enable {
    ...
    postCreate =
      ...
      then ''
        # export BORG_PASSCOMMAND="cat ${config.sops.secrets.borgRepoKey.path}"
        # export BORG_RSH="ssh -i /root/.ssh/id_ed25519 -o StrictHostKeyChecking=no"
